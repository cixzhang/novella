...
function constructGeneral(filepath, { filename, index, output }) {
  ...
  return { index, name, filename, location: output, mtime, src: `${output}/${filename}` };
}

function constructImage(filepath, { filename, output }) {
  return files.statImage(filepath)
    .then(([width, height]) => (
      { type: 'image', width, height, thumb: `thumbs/${filename}` }
    ));
}
