var Novella = (function (Vue) {
  ...
  console.log("Generated by novella at 10/24/2016, 10:33:36 PM.");
  ...
  var Gallery = { template: "<div class=\"gallery\"><button class=\"sidebar-toggle\" v-on:click=\"toggleSidebar()\">{{ store.showsidebar ? '« hide pages' : '» show pages' }}</button><header><span>{{ store.title }}</span> <span>{{ getPage(store.pagenum).name }}</span></header><page :page=\"getPage(store.pagenum)\"></page><page-control :store=\"store\"></page-control></div>",
    props: { store: store$1 },
    components: {
      Page,
      PageControl,
