var Novella = (function (Vue) {
  ...
  console.log("Generated by novella at 10/23/2016, 12:12:14 AM.");
  ...
  var PageControl = { template: "<div class=\"page-control\"><a class=\"prev\" :href=\"getRoute(store.pagenum - 1)\" :disabled=\"!canPrev()\">Prev </a><a class=\"next\" :href=\"getRoute(store.pagenum + 1)\" :disabled=\"!canNext()\">Next</a></div>",
    props: { store: store$1 },
    data: () => ({
      animateSidebar: null,
      hideSidebar: false,
  ...
  var App = { template: "<div class=\"app\" :data-close-sidebar=\"hideSidebar && !animateSidebar\"><div class=\"sidebar\" :data-animation=\"animateSidebar\" v-on:animationend=\"onAnimationEnd()\"><h1>{{ store.title }}</h1><page-list :store=\"store\"></page-list></div><div class=\"content\"><button class=\"sidebar-toggle\" v-on:click=\"toggleSidebar()\">{{ hideSidebar ? 'show menu' : 'hide menu' }}</button><page :page=\"getPage(store.pagenum)\"></page><page-control :store=\"store\"></page-control></div></div>",
    props: { store: store$1 },
    data: () => ({
      animateSidebar: null,
      hideSidebar: false,
